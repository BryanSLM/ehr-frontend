...
export class CitasService {
  ...
  getDoctores(): Observable<Doctor[]> {
    return this.http
      ...
      .pipe(
        map((response) => {
          console.log(
            'Respuesta completa de doctores:',
  ...
  getConsultorios(): Observable<Consultorio[]> {
    return this.http
      ...
      .pipe(
        map((response) => {
          console.log(
            'Respuesta completa de consultorios:',
            JSON.stringify(response, null, 2),
          );
          if (Array.isArray(response)) {
            return response;
  ...
  findConsultorioByDoctorId(doctorId: number): Observable<Consultorio | null> {
    return this.getConsultorios().pipe(
      map((consultorios) => {
        ...
        console.log(
          `Consultorio encontrado para doctor ${doctorId}:`,
          consultorio,
        );
  ...
  getAllCitas(): Observable<Cita[]> {
    return this.http
      .get<ApiResponse<Cita[]>>(this.citasUrl, {
        headers: this.getHeaders(),
      })
      .pipe(
        map((response) => {
  ...
  getCitaById(id: number): Observable<Cita> {
    return this.http
      .get<ApiResponse<Cita>>(`${this.citasUrl}/${id}`, {
        headers: this.getHeaders(),
      })
      .pipe(
        map((response) => {
  ...
  createCita(citaData: CitaCreate): Observable<Cita> {
    return this.verificarDisponibilidad(
      citaData.fecha,
      citaData.hora,
  ...
  updateCita(id: number, citaData: Partial<CitaCreate>): Observable<Cita> {
    return this.http
      .put<ApiResponse<Cita>>(`${this.citasUrl}/${id}`, citaData, {
        headers: this.getHeaders(),
      })
      .pipe(
        map((response) => {
  ...
  deleteCita(id: number): Observable<void> {
    return this.http
      .delete<ApiResponse<void>>(`${this.citasUrl}/${id}`, {
        headers: this.getHeaders(),
      })
      .pipe(
        map((response) => {
  ...
  getCitasByPaciente(pacienteId: number): Observable<Cita[]> {
    return this.http
      .get<ApiResponse<Cita[]>>(`${this.citasUrl}/paciente/${pacienteId}`, {
        headers: this.getHeaders(),
      })
      .pipe(
        map((response) => {
          if (!response.success) {
            throw new Error(
              response.message || 'Error al obtener las citas del paciente',
            );
          }
          return response.data || [];
        }),
  ...
  getCitasByDoctor(doctorId: number): Observable<Cita[]> {
    return this.http
      .get<ApiResponse<Cita[]>>(`${this.citasUrl}/doctor/${doctorId}`, {
        headers: this.getHeaders(),
      })
      .pipe(
        map((response) => {
          if (!response.success) {
            throw new Error(
              response.message || 'Error al obtener las citas del doctor',
            );
          }
          return response.data || [];
        }),
  ...
  updateEstadoCita(
    ...
    return this.http
      .patch<ApiResponse<Cita>>(
        `${this.citasUrl}/${id}/estado`,
        { estado },
        {
          headers: this.getHeaders(),
        },
