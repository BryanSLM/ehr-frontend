...
export class PdfService {
  private async agregarEncabezado(doc: jsPDF): Promise<number> {
    ...
    return new Promise<number>((resolve) => {
      img.onload = () => {
        ...
        doc.text(
          'Teléfonos: (555) 123-4567 / (555) 765-4321',
          margin + imgWidth + 10,
          margin + 21,
        );
        doc.text(
          'Email: info@clinicacardiovascular.com',
          margin + imgWidth + 10,
          margin + 28,
        );
  ...
  async generarRecetaMedica(evolucion: any, paciente: any): Promise<void> {
    ...
    doc.text(
      `Paciente: ${paciente.primer_nombre} ${paciente.apellido_paterno}`,
      margin,
      currentY,
    );
    doc.text(`Cédula: ${paciente.cedula}`, margin, currentY + 7);
    doc.text(
      `Fecha: ${new Date(evolucion.fecha).toLocaleDateString()}`,
      margin,
      currentY + 14,
    );
    currentY += 25;
    ...
    // Firma del médico
    doc.setFontSize(12);
    doc.text('_____________________', pageWidth - 60, pageHeight - 15, {
      align: 'center',
    });
    doc.text('Firma del Médico', pageWidth - 60, pageHeight - 10, {
      align: 'center',
    });

    doc.save(`receta_${paciente.cedula}_${new Date().getTime()}.pdf`);
  }
  async generarCertificadoMedico(
    evolucion: any,
    paciente: any,
    datos: any,
  ): Promise<void> {
    ...
    // Título
    doc.setFontSize(16);
    doc.text('CERTIFICADO MÉDICO', pageWidth / 2, startY + 10, {
      align: 'center',
    });
    ...
    // Motivo del reposo
    doc.text(
      'Por medio de la presente certifico que el paciente:',
      margin,
      currentY,
    );
    currentY += 10;
    ...
    const motivo =
      `Presenta ${evolucion.enfermedad_actual} que requiere un período de reposo para su adecuada recuperación. ` +
    ...
    // Texto de firma
    doc.text('Firma y Sello del Médico', firmaX, currentY + 10, {
      align: 'center',
    });
