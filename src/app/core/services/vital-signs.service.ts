...
export class VitalSignsService {
  ...
  private pacientesUrl = `${environment.apiUrl}/patients`;
  ...
  update(id: number, vitalSigns: any): Observable<any> {
    return this.http
      .put(`${this.apiUrl}/${id}`, vitalSigns)
      .pipe(catchError(this.handleError('Error al actualizar signos vitales')));
  }

  delete(id: number): Observable<any> {
    return this.http
      .delete(`${this.apiUrl}/${id}`)
      .pipe(catchError(this.handleError('Error al eliminar signos vitales')));
  }

  getById(id: number): Observable<any> {
    return this.http
      .get(`${this.apiUrl}/${id}`)
      .pipe(catchError(this.handleError('Error al obtener signos vitales')));
  }
  ...
  getCompletedVitalSigns(filters?: any): Observable<any> {
    ...
    return this.http.get(`${this.apiUrl}/records/completed`, { params }).pipe(
      tap((response) =>
        console.log('Respuesta registros completados:', response),
      ),
      map((response: any) => {
        if (response?.success && Array.isArray(response.data)) {
          ...
          if (filters?.searchTerm) {
            ...
            filteredData = filteredData.filter((record: any) => {
              const nombreCompleto =
                `${record.paciente?.primer_nombre} ${record.paciente?.apellido_paterno}`.toLowerCase();
          ...
          if (filters?.fecha) {
            filteredData = filteredData.filter((record: any) => {
              const recordDate = new Date(record.fecha_medicion)
                .toISOString()
                .split('T')[0];
