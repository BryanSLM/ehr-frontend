...
export class EvolucionMedicaComponent implements OnInit {
  ...
  onSubmit() {
    console.log(
      ...
      this.evolucionForm.valid,
      this.diagnosticoForm.valid,
    );
    ...
    if (this.evolucionForm.valid && this.diagnosticoForm.valid) {
      ...
      const evolucionData = {
        ...
        antecedentes_personales: this.evolucionForm.get(
          'antecedentes_personales',
        )?.value,
        antecedentes_familiares: this.evolucionForm.get(
          'antecedentes_familiares',
        )?.value,
        pacienteId: Number(this.pacienteId),
        medicoId: Number(this.medicoService.obtenerMedicoActualId()),
        signosVitalesId: this.signosVitales?.id
          ? Number(this.signosVitales.id)
          : null,
      ...
      if (
        ...
        if (!evolucionData.enfermedad_actual)
          this.error += ' enfermedad_actual,';
        if (!evolucionData.antecedentes_personales)
          this.error += ' antecedentes_personales,';
      ...
      const diagnosticosInvalidos = evolucionData.diagnosticos.some(
        (diag: any) => !diag.cieId || !diag.tipo,
      );

      if (diagnosticosInvalidos) {
        this.error =
          'Todos los diagnósticos deben tener un código CIE y un tipo seleccionado';
        this.loading = false;
        return;
      }
      ...
      if (
        !evolucionData.medicamentos ||
        evolucionData.medicamentos.length === 0
      ) {
      ...
      const medicamentosInvalidos = evolucionData.medicamentos.some(
        (med: any) =>
          !med.fecha_emision ||
          !med.nombre_generico ||
          !med.concentracion ||
      ...
      if (medicamentosInvalidos) {
        this.error =
          'Todos los medicamentos deben tener los campos requeridos completos';
        this.loading = false;
        return;
      }
      ...
      const datosFinales = {
        ...
        medicamentos: evolucionData.medicamentos.map((med: any) => ({
          ...med,
          fecha_emision:
            med.fecha_emision || new Date().toISOString().split('T')[0],
        })),
      };
      ...
      const request = this.isEditMode
        ? this.medicoService.actualizarEvolucion(
            this.evolucionId!,
            datosFinales,
          )
      ...
      this.error =
        'Por favor, complete todos los campos requeridos incluyendo al menos un diagnóstico y un medicamento';
    }
  }
      ...
      this.medicoService.obtenerEvolucion(this.evolucionId).subscribe({
        next: (response) => {
          if (response.data) {
            ...
            // Cargar diagnósticos
            const diagnosticosArray = this.diagnosticoForm.get(
              'diagnosticos',
            ) as FormArray;
            diagnosticosArray.clear();
            if (
              response.data.diagnosticos &&
              response.data.diagnosticos.length > 0
            ) {
            ...
            const medicamentosArray = this.prescripcionForm.get(
              'medicamentos',
            ) as FormArray;
            medicamentosArray.clear();
            if (
              response.data.medicamentos &&
              response.data.medicamentos.length > 0
            ) {
              response.data.medicamentos.forEach((medicamento: Medicamento) => {
                medicamentosArray.push(
                  this.fb.group({
                    ...
                    indicaciones_adicionales: [
                      medicamento.indicaciones_adicionales,
                    ],
                  }),
                );
              });
        ...
        next: (response) => {
          ...
          if (paciente) {
            ...
            if (paciente.signosVitales && paciente.signosVitales.length > 0) {
              ...
              this.error =
                'El paciente debe tener signos vitales registrados para crear una evolución';
            }
          } else {
