...
export class CitaFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.citaForm
      .get('especialidad')
      ?.valueChanges.subscribe((especialidad) => {
    ...
    this.citaForm.get('doctorId')?.valueChanges.subscribe((doctorId) => {
      if (doctorId) {
        const doctorSeleccionado = this.doctores.find(
        ...
        if (doctorSeleccionado) {
          ...
          const consultorioAsignado = this.consultorios.find(
            (c) => c.doctorId === doctorSeleccionado.id,
          );
          ...
          if (consultorioAsignado) {
            ...
            this.consultorioAsignado = `Consultorio ${consultorioAsignado.numero}${
              consultorioAsignado.descripcion
                ? ' - ' + consultorioAsignado.descripcion
                : ''
            }`;
            ...
            this.citasService
              .getCitasByConsultorio(consultorioAsignado.id)
              .subscribe({
                next: (citas) => {
                  this.citasExistentes = citas;
                  console.log(
                    'Citas existentes cargadas:',
    ...
    this.citaForm.get('fecha')?.valueChanges.subscribe((fecha) => {
      ...
      if (fecha) {
        ...
        if (doctorId && consultorioId) {
          this.loading = true;
          this.citasService
            .getHorariosDisponibles(consultorioId, fecha, doctorId)
            .subscribe({
              next: (horarios) => {
  ...
  onSubmit() {
    if (this.citaForm.valid) {
      ...
      if (this.isEditing) {
        // Si estamos editando
        this.citasService
          .updateCita(this.route.snapshot.params['id'], citaData)
          .subscribe({
            next: (response) => {
